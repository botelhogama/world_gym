...
start_time = time()
members_data = get_data.GetClientData(endpoint="/api/v1/members", take_number=50)
df_members = members_data.retrieve_data(use_cache=False)
print(f"df_members shape is : {df_members.shape}")

membership_data = get_data.GetClientData(endpoint="/api/v1/membermembership", take_number=25)
df_membership = membership_data.retrieve_data(use_cache=True)
print(f"df_membership shape is : {df_membership.shape}")
...
data.to_csv('C:\\Users\\Pichau\\PycharmProjects\\worldgym\\data\\data_raw.csv', index=True, header=True)
print(Fore.GREEN + "file saved as data_raw.csv")
